...
import bing.security.CustomBasicAuthenticationEntryPoint;
...
public class MultiHttpSecurityConfig {
	...
	public static class ApiWebSecurityConfigurationAdapter extends WebSecurityConfigurerAdapter {
		...
		protected void configure(HttpSecurity http) throws Exception {
			LOGGER.info("********************* ApiWebSecurityConfigurationAdapter *************************");
			http.csrf().disable();
			// 当前权限控制只拦截/api开头的URI
			http.antMatcher("/api/**").authorizeRequests().antMatchers("/api/**/authenticate").permitAll().anyRequest().authenticated().and().httpBasic().realmName(SecurityConstants.REALM_NAME)
					// 配置未登录请求资源的响应处理类
